...
- **응답 유실 시 읽기 명령의 오류 메시지가 사실과 다르다.** 재시도 정책 자체는
  읽기/조작을 구분한다(읽기는 3회 재시도). 다만 `WDA_RESPONSE_LOST` **메시지**가
  조작 기준으로 고정돼 있어, `screenshot` 실패 시 "조작이 적용됐을 수 있다"(적용될
  조작이 없다)와 "재시도를 수행하지 않았다"(이미 3회 했다)가 함께 나온다.
  동작이 아니라 안내가 틀린 것이며, 수동 재호출은 즉시 성공한다.
