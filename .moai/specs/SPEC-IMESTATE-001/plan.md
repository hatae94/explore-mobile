...
- **레코드 생성은 배타 생성**(`wx`)으로 한다. **`EEXIST`에 한해** 오류가 아니라 "다른 프로세스가 이겼다"는 정상 결과로 처리하고 기존 값을 보존한다(REQ-007 / AC-021·022). **비-`EEXIST` 실패(`ENOENT`·`EACCES`·`ENOSPC`)는 삼키지 않는다** — `try { wx } catch { }`로 뭉치면 기록 없이 정상 종료해 이 SPEC이 없애려는 결함과 같은 조용한 실패가 생긴다(AC-027).
...
- **구 파일 기록만 있는 시리얼의 `clear`도 툼스톤을 만든다**(REQ-005 / AC-025). 레코드가 없다고 no-op으로 처리하면 이관 창 좀비가 열린다.
...
- **이관 단위는 일괄이다**(`spec.md` §A.3-⑧): 구 파일의 모든 항목을 신규 레코드로 옮긴 뒤 구 파일을 폐기한다. 시리얼별 지연 이관은 채택하지 않는다 — 0.2.0은 단위를 규정하지 않았고, 두 선택은 관측 결과가 달랐다(2차 감사 N-MF-2).
- **이관은 툼스톤 있는 시리얼을 건너뛴다**(툼스톤 게이트 / AC-026). 조회 1단계(레코드)가 2단계(툼스톤)보다 앞서므로, 게이트 없이 레코드를 만들면 억제가 무력화된다 — 감사관이 명세대로 시뮬레이션해 재현했다. 건너뛴 항목이 구 파일 폐기와 함께 사라지는 것은 정확한 동작이다.
