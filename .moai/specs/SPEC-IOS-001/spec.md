...
- **REQ-IOS-BACKEND-006** (When 이벤트) — *2026-07-25 실기기 검증으로 개정*: **When** `IdbBackend.inputText`가 호출될 때, it **shall** 텍스트가 `idb ui text`로 인코딩 가능한 범위(출력 가능 ASCII + 개행)이면 `idb ui text "<...>"` **단일 호출**로 입력하고, 그 외에는 `simctl pbcopy`로 기기 페이스트보드에 기록한 뒤 Command(HID 227) 홀드 + V(HID 25)로 붙여넣는다. **개정 사유**: `idb ui text`는 Unicode-native가 **아니다** — fb-idb 1.1.7 `idb/common/hid.py`의 `text_to_events`가 고정 미국 자판표만 처리하고 그 외 문자에 `No keycode found for 네` 예외를 던져, 한글·이모지 입력이 불가능하다(최초 전제는 실측으로 반증됨). 그럼에도 Android 대비 단순화는 유지된다: ADBKeyBoard APK/GPL 다운로드/base64 broadcast/디스크 영속 IME 세션이 **모두 불필요**하다(§C.2). `options.hideKeyboardAfter`는 iOS에서 대응 동작이 없어 no-op이다(관찰 가능한 무해 무시).
