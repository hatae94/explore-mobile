...
- **b12_self_test_b** (AC count match, acceptance.md SSOT 대조): PASS — `grep -cE '^\| AC-IOS-[0-9]+ \|' acceptance.md` = 28 (§D 매트릭스 28건) = progress.md §E.2 요약(PASS 19 + PASS-WITH-DEBT 9 + FAIL 0 = 28, sync-auditor F1 BLOCKER 해소로 AC-IOS-017이 PASS-WITH-DEBT → PASS 이동한 이후 수치)과 일치, CHANGELOG Notes 섹션의 "19 PASS, 9 PASS-WITH-DEBT, 0 FAIL" 서술과도 일치(이번 재조정 커밋에서 CHANGELOG/README의 18/10/292 잔존 수치를 19/9/293으로 정정 완료)
