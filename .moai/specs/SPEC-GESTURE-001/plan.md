...
### B.7 가드가 자기 화면에서만 발동했다 [최고 — 0.5.0 amendment의 존재 이유]
...
### M7 — 움직임 가능성 술어 + 문턱 측정 [0.5.0 amendment · 측정이 산출물이다]

> **선행**: M6 완료(0.4.0에서 마감). 재감사 0.76 / SAFE TO PUSH: No가 근거(B.7).

**산출물 1 — 문턱 측정(다른 모든 산출물의 선행).**

`MIN_EFFECTIVE_SWIPE_PX`를 **측정한다. 고르지 않는다.** 이것은 코드 작업이 아니라 실측 작업이며 **M7의 첫 번째 산출물**이다.
...
- 알려진 출발점(희소 4점): 2px → 0/3, 4px → 1/3, ~44px → 3/3, ~437px → 3/3. 하한은 4px과 44px 사이 어딘가다.
