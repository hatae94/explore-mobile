...
| `src/cli/commands/scroll-geometry.ts` | M6·M7·M8·**M10** | **M10 NN5 — `ratio=1` 전제 주석의 거짓 근거 정정 + 넘을 수 있는 비율이 없는 화면의 처분**(자기거부 권고 금지) |
| `src/cli/commands/scroll-geometry.test.ts` | M7·M8·**M10** | **M10 AC-GEST-034 픽스처** — mock 문턱·mock 화면으로 권고값 왕복 + 권고 부재 경로 |
| `src/cli/commands/scroll.ts` | M6·M7·M8·**M10** | **M10 — 권고 부재를 응답에 반영**(`minValidRatio` 미탑재 경로). 거부 자체와 무제스처 보장은 불변 |
| `src/cli/commands/scroll.test.ts` | M8·**M10** | **M10 — 권고 부재 응답 단언** |
| `src/backend/adb-backend.ts` | M1·M8·M9·**M10** | **M10 NN10(선택) — 문턱 캐싱.** 넣는다면 유효 밀도 변경 시 무효화 필수(REQ-GEST-SCROLL-008 0.8.0 주석) |
| `src/backend/adb-backend.test.ts` | M8·M9·**M10** | **M10 — 캐시를 넣은 경우에만**: 밀도 변경 후 갱신값 반환 픽스처 |
...
> **선행**: M1~M9 완료(0.7.0에서 마감·푸시, 직전 completed `392c9b2`). 이 마일스톤은 **새 능력이 아니라 부채 정리**로 열렸다 — 다섯 번째 감사가 MUST-FIX 0건·**0.89 / SAFE TO PUSH: Yes**를 내면서 지적한 것은 개별 결함이 아니라 **패턴**이었다: 이월 6건 중 5건이 1~3줄인데 docs 패스가 열려 있어도 쓸리지 않는다.
>
> **PATH 전제**: M8·M9와 동일 — `export PATH="$HOME/Library/Android/sdk/platform-tools:$PATH"`.
>
> **⚠️ 디스플레이 밀도를 바꾸지 않는다.** 0.7.0에서 복원됐고(`Physical density: 600` 단일 행), 이 마일스톤의 파생 검증은 전부 픽스처로 한다.
>
> **⚠️ 이 마일스톤에는 새 REQ가 없다.** 산출물 전부가 기존 REQ의 주석·기존 AC의 정정·§C.1의 사실 기록 위에 선다. **범위를 늘려 잡지 않는다** — 부채 정리에서 SPEC이 커지면 다음 라운드에 또 이월된다.

**아홉 건의 배분(하나도 흘리지 않는다).** 5차 감사의 SHOULD-FIX는 9건이며 세 주체로 갈린다.

| 주체 | 항목 | 상태 |
|------|------|------|
| **manager-spec(0.8.0, 완료)** | NF1(AC-027 수치) · NF2(안정화 지연) · NN9(`basis` 서술) | 본 amendment에서 닫힘 |
| **M10(구현)** | NF3 · NN5 · NN10 · NN6 | 아래 산출물 1~4 |
| **docs 패스** | NN3(README:355 iOS 절반) · `--web` 프록시 불안정성 고지 · **NN9 후속**(README:399의 `"device-query"` 서술) | 아래 "docs 패스로 넘기는 것" |

**산출물 1 — 권고 부재 경로(NN5의 규범 절반) [최고 변경 확률 · 사용자 대면].**

응답에서 필드 하나가 사라지는 경우가 생기므로 **관측 가능한 계약이 바뀐다.** 되돌리기 가장 비싸고, 이 목록에서 유일하게 규범적 판단이 걸려 있다.

- 문턱을 넘는 비율이 **존재하지 않는 화면**에서는 `minValidRatio`를 **싣지 않는다**(spec.md REQ-GEST-SCROLL-007 0.8.0 주석). 이분 탐색이 `hi=1`을 돌려주는 현재 거동은 **자기 자신이 거부할 값을 권한다** — 이 SPEC이 0.5.0·0.6.0에서 두 번 제거한 계열의 **세 번째 입구**다.
- **거부 자체와 무제스처 보장은 불변** — `AMOUNT_TOO_SMALL`은 그대로 나가고 제스처는 전송되지 않는다. 사라지는 것은 권고뿐이다.
- **부재의 표현 형태는 구현 판단**이다(필드 생략 / 명시적 부재 표기 등, spec.md §D "구현 세부(HOW)"). 다만 **없는 값을 지어내지 않는다**는 성질은 규범이다.
- **RED 먼저**: `12×12` 화면 + 문턱 32px 픽스처에서 현재 구현이 `1`(자기거부값)을 돌려주는 것을 확인한 뒤 고친다.
- **AC-GEST-034.** 기기 불필요 — 문턱이 인자로 주입되는 순수 함수 계층이라 mock만으로 판정된다(M8 산출물 2).

**산출물 2 — `behavior:"instant"` 절벽(NF3) [높음 · 트레이드오프가 있다].**

- **SPEC은 버전 하한을 선언하지 않는다**(spec.md §D 신설). 즉 "Safari 17.4 이상 지원"을 코드나 문서에 적는 선택지는 **없다.**
- **방어적 폴백 채택 여부는 구현 판단이다.** 넣지 않아도 SPEC 위반이 아니다 — 현재 지원 구성(iOS 18.6 / 26.0)은 영향받지 않음이 실측됐다(§C.1-㉓).
- **넣는다면 대가를 주석에 남긴다**: `behavior` 인자 없는 폴백 호출은 페이지 CSS `scroll-behavior: smooth`를 따르므로 **M9가 닫은 비동기 표본 결함이 그 경로에서 되살아난다.** 강등 범위가 "모든 화면 밖 요소" → "구버전 WebKit + smooth 페이지"로 줄어드는 개선이지 **동등이 아니다.** "폴백으로 해결됨"이라고 적으면 이 SPEC이 네 번 지운 과장을 다시 만든다.
- 폴백을 넣은 경우에만 픽스처 추가: 열거값을 거부하는 WebKit stub에서 폴백 경로가 실제로 도는지.

**산출물 3 — 문턱 캐싱(NN10) [중간 · 선택].**

- 캐시 유무·자료구조·키·무효화 계기는 **전적으로 구현 판단**이다. 넣지 않아도 SPEC 위반이 아니다.
- **넣는다면 무효화는 규범이다**(spec.md REQ-GEST-SCROLL-008 0.8.0 주석). 유효 밀도는 사용자가 디스플레이 크기를 바꾸면 **실제로 변한다**(§C.1-⑳ 실측). 오래된 캐시의 위험은 비대칭이다 — **확대 방향**에서 캐시값이 새 슬롭보다 낮아지고, 그 구간은 수용돼 전송된 뒤 **탭으로 해석된다**(§C.1-⑱).
- 즉 **캐시는 성능 최적화이면서 동시에 새 결함 표면**이다. `ImeSessionStore`·`calibration.ts`의 기기별 캐시 선례가 있으나, 그것들과 달리 **이 값은 세션 중에 바뀔 수 있다** — 선례를 그대로 베끼지 않는다.
- 넣은 경우에만 픽스처 추가: 밀도가 바뀐 뒤 갱신된 값을 돌려주는지.

**산출물 4 — 사실 오류·주석 정리 [최저 · 동작 변화 0].**

- **NN5 주석**(`scroll-geometry.ts`): `ratio=1` 전제를 "그런 화면은 이미 REQ-GEST-SCROLL-004가 거부한다"로 정당화하고 있는데 **거짓이다** — `deriveScreenSize([{0,0,12,12}])`는 원점 witness를 갖춘 정상 파생이라 `{12,12}`를 반환한다. 주석을 사실에 맞게 고친다(전제가 무엇에 의존하는지 정직하게: 실기기 화면 크기 가정).
- **NN6**(`swipe.ts`): `@MX` 태그가 **0개**다. 종류·위치는 구현 판단이며 SPEC 요구사항이 아니다.
- 이 산출물은 **테스트를 바꾸지 않는다** — 644건 기준선 그대로여야 한다.

**산출물 5 — 회귀.**

- **644건 기준선**(29파일) 감소 없이 통과, `pnpm typecheck` / `pnpm build` exit 0. 산출물 1을 넣으면 신규 테스트만큼 증가한다.
- **iOS·Android 문턱값 불변 확인** — 이 마일스톤은 문턱 *산식*을 건드리지 않는다. `minValidRatio`가 iOS `0.013984236866235733`, 실측 기기 Android `0.011039886623620987`로 그대로여야 한다.

**docs 패스로 넘기는 것(M10 아님).**

- **NN3** — `README.md:355`의 "Below that many device pixels nothing happens"는 **iOS 절반이 틀렸다.** 9pt 1/15·10pt 2/15로 *가끔* 움직였으므로 "아무 일도 없다"가 아니라 **"이동을 신뢰할 수 없다"**가 맞다(§C.1-⑭). Android 절반("something *else* happens")은 정확하다.
- **`--web` 프록시 불안정성** — `tap --web` 참조 절에 고지가 **0회**다. 완화책(호출 사이 5-6초 간격)은 M5가 이미 실측했고 감사가 세 라운드 연속 이 때문에 시간을 썼다.
- **NN9 후속** — `README.md:399`의 `"device-query"` 설명 *"Queried from this device at call time"*은 spec.md가 0.8.0에서 정정한 그 과장이다. **응답 스키마는 바뀌지 않으므로** 예시 JSON은 그대로 두고 **산문만** 고친다: 기기에서 읽은 값이 아니라 **기기 조회로 매개화된 플랫폼 규칙**.
...
M9 ──> M10 ┬─ 권고 부재 경로   (기하·CLI)   [규범 · 사용자 대면]
           ├─ `instant` 폴백    (웹 경로)    [선택 · 트레이드오프]
           ├─ 문턱 캐싱        (Android 백엔드)[선택 · 새 결함 표면]
...
**M10은 앞의 넷과 성격이 다르다 — 결함도 새 관측도 아니고, 쌓인 부채다.** M6·M7은 감사가, M8은 새 기기가, M9는 자기 문서의 열린 질문이 열었다. M10을 연 것은 **다섯 라운드에 걸쳐 이월된 목록 그 자체**이며, 5차 감사는 개별 항목보다 그 이월 패턴을 문제로 지목했다 — *"docs 패스가 이미 열려 있는데도 이월 LOW 항목이 쓸리지 않는다."* 이 마일스톤이 앞의 넷과 달라야 하는 지점은 **범위를 늘리지 않는 것**이다: 부채 정리에서 SPEC이 커지면 다음 라운드에 또 이월된다. 그래서 M10에는 신규 REQ가 없고, 신규 AC도 2건뿐이며, 산출물 넷 중 둘은 명시적으로 **선택**이다.
