...
| ⑭ | **`MIN_EFFECTIVE_SWIPE_PX` — 화면을 움직이는 최소 스와이프 거리** | **미측정 — plan.md §F M7의 산출물.** 현재 알려진 것은 희소한 4점뿐이다: 2px → 3회 중 0회, 4px → 3회 중 1회, ~44px → 3/3, ~437px → 3/3. 하한은 이 넓은 구간 어딘가에 있다. 값이 채워지기 전까지 REQ-GEST-SCROLL-007은 미충족이다 | **미측정(M7 대상)** |
