...
| 0.5.0 | 2026-07-28 | hatae | **in-place amendment** — 재감사 **0.69 → 0.76, 여전히 SAFE TO PUSH: No**. 0.4.0의 퇴화 가드가 **축 길이가 홀수면 아예 발동하지 않고**, 그 대안으로 제시하던 최소 비율이 **움직이지 않는 값**을 돌려준다. 아래 `## Amendments` 참조. |
...
- **REQ-GEST-SCROLL-007** (When 감지된-이상상태): **When** REQ-GEST-SCROLL-001~003으로 계산한 스와이프가 **화면을 움직일 수 없을 때** — 즉 `from`과 `to`의 스크롤 축 거리가 **최소 유효 이동 거리 `MIN_EFFECTIVE_SWIPE_PX` 미만**일 때 — the CLI **shall** `AMOUNT_TOO_SMALL`로 거부하고 **어떤 제스처도 전송하지 않는다.** 응답에는 거부된 비율과 **그 화면에서 이 문턱을 넘는 최소 비율**을 함께 싣는다.
  ...
  - **왜 거부인가(0.4.0 설계 결정, 유지).** 좌표는 정수 픽셀이어야 하므로(`parseCoordinate`의 `^\d+$`) 최종 단계에 반올림이 들어간다. 402x874 화면에서 `--amount 0.001`은 이동 거리 0.79px를 만들고 양끝이 같은 픽셀(437)로 접혀 **거리 0인 스와이프**가 된다 — 그런데 응답은 `ok:true`다. 실측: `0.001`·`0.0012` → `from.y=437 to.y=437`, `scrollY 3212→3212`, 스크린샷 바이트 동일. `0.002`부터 정상(438→436).
...
- **작은 `--amount`는 이 화면에서 거부된다.** 이동 거리가 `MIN_EFFECTIVE_SWIPE_PX` 미만인 비율은 `AMOUNT_TOO_SMALL`이다(REQ-GEST-SCROLL-007). 임계 비율은 화면 크기에 따라 달라지며, **문턱값 자체가 아직 미측정이다**(§C.1-⑭, M7).
- **터치 슬롭은 플랫폼 속성이라 여기서 Android를 알 수 없다.** M7이 측정하는 `MIN_EFFECTIVE_SWIPE_PX`는 **iOS 값**이다. Android는 `adb`가 없어 측정 자체가 불가능하며(§C.2), iOS 값을 Android 근거로 쓰지 않는다.
...
- `MIN_EFFECTIVE_SWIPE_PX`를 **추론·문서·유추로 정하지 않는다**(REQ-GEST-SCROLL-007). 측정되기 전에는 미충족 상태로 둔다.
