...
| A2 | **AC-VISION-008의 `--index` 절** | ~~명시적 미충족~~ → **해소됨 (2026-08-03)** | 당시 판단: `--index`는 웹 전용 플래그로 **존치**, 제거하면 SPEC-WEBVIEW-001이 깨진다(§G 「`--index` 플래그 제거 여부」). **그 상한이 사라졌다** — `SPEC-WEBVIEW-002`가 `--web` 경로 전체를 제거하면서 `--index`의 유일한 소비자가 없어졌고, 플래그도 함께 제거됐다. 이제 `--id`·`--index` 두 절 모두 충족 |
...
| B2 | **CLI 자체 프록시 기동 결함** — CLI가 띄운 프록시로는 `tap --web`이 3/3 실패 | ~~SPEC-WEBVIEW-001~~ → **소멸 (2026-08-03)** | `proxy-service.ts:288-296`의 기동 루프가 첫 조회 빈 목록에서 재시도 없이 종료한다(추론 — CLI 자체를 계측하지는 않음). **`SPEC-WEBVIEW-002`가 `--web` 경로를 통째로 제거하면서 이 결함이 서술하던 코드 자체가 없어졌다.** 함께 `AC-VISION-031`의 조건부 PASS도 대상이 사라졌다. §E.2 「AC-VISION-031 검증」은 이력으로 남긴다 |
