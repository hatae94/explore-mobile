...
| 버전 | 날짜 | 작성자 | 변경 내용 |
|------|------|--------|-----------|
| 0.6.0 | 2026-08-05 | hatae | **마감 후 실기기 관측 반영 — AC-READY-013이 닫혔다. 요구사항·설계·코드 무변경.** 0.5.2 마감 시점에 미관측으로 남았던 AC-READY-013(실기기 중복 전송)을 마감 커밋 `596a798` 이후 실기기에서 관측했고 ①②③④ 전부 성립했다. **직전 판이 미관측 사유로 적은 전제 하나가 틀렸다** — `progress.md`는 "두 번째 전송을 만들려면 사용자가 폰에서 무선 디버깅을 켜야 하고, 오케스트레이터가 대신 할 수 없다"고 적었으나, 이 폰은 Android 10이라 **개발자 옵션에 무선 디버깅 토글 자체가 없다**(`adb_wifi_enabled` → `null`; 이 키는 Android 11부터). 실제 경로는 USB 연결 위의 `adb tcpip`이었고 사용자 조작이 필요 없었다. 관측 전문은 `.moai/reports/android-verification/SPEC-READY-001-android-2026-08-05.md`, 판정 근거는 `progress.md` §J. 과거 미관측 기록은 **지우지 않고 보존**했다 — 그 시점에는 사실이었고, §J가 시점을 명시해 이어받는다. |
