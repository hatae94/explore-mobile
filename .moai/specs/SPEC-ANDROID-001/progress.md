...
### AC 매트릭스 — M2 커밋 시점 스냅샷 (§D 19건 전체 — 이번 청크 범위 외 항목은 PENDING)
...
| AC-ANDROID-014 | 3계층 아키텍처 유지 | REQ-ARCH-002 | design-review | PARTIAL | 정규화 계층(M2)이 CLI/adb 래퍼 어느 쪽에도 의존하지 않음을 확인(`src/normalize/uiautomator.ts`는 `src/schema/*`만 import). CLI 계층(M3)·adb 래퍼(M4)가 아직 없어 전체 3계층 경계의 최종 확인은 보류 |
...
**요약 (M0+M1+M2 청크 완료 시점)**: 19건 중 PASS 2건(AC-006, AC-008), PARTIAL 2건(AC-012, AC-014), PENDING 15건(M3-M8 후속 위임 범위). FAIL 0건. 이번 청크에서 시도된 항목 중 미검증 상태는 없음(verification-claim-integrity 원칙 준수 — 미구현 항목은 PASS로 주장하지 않고 PENDING으로 명시).
...
$ pnpm vitest run --reporter=verbose
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > maps class->role, resource-id->id, bounds->{x,y,w,h}, clickable+enabled->tappable, and nests children (REQ-SCHEMA-002) 2ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > prefers a non-empty text attribute over content-desc (REQ-SCHEMA-002 text/content-desc->text) 0ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > preserves unicode text (Korean + emoji) without corruption 0ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > defaults bounds to {x:0,y:0,w:0,h:0} when the bounds attribute is missing or malformed (acceptance.md §D.1) 0ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > returns an empty array for an empty or whitespace-only XML string (acceptance.md §D.1 edge case) 0ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > returns an empty array for a hierarchy with no child nodes 0ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > never throws on malformed/truncated XML and returns a safe (possibly partial) array (acceptance.md §D.1) 0ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > is defensive against non-string input at the runtime boundary (Secured — external device output) 0ms
 ✓ src/normalize/uiautomator.test.ts > normalizeUiAutomatorXml > requires BOTH clickable AND enabled to derive tappable=true 0ms
...
Branches     : 92.85% ( 26/28 )
Functions    : 100% ( 6/6 )
Lines        : 96.77% ( 30/31 )
(threshold: 85%+ per acceptance.md §D.2 — MET)

$ pnpm build
(exit=0; dist/index.js, dist/cli/bin.js, dist/schema/*.js, dist/normalize/uiautomator.js emitted)
```
