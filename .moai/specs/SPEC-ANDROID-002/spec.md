...
`parseAdbDevicesList`가 adb의 실제 출력 계약(탭 구분)을 따라 serial과 state를
분리한다. serial에 공백이 있어도 온전히 보존되고 state가 정확히 읽힌다.
...
### REQ-SERIAL-001 — 탭을 구분자로 삼는다

**Where** `adb devices` / `adb devices -l` 출력 한 줄을 파싱할 때,
**the system shall** serial과 state를 **탭**으로 분리하여, serial 안의 공백을
serial의 일부로 보존한다.

### REQ-SERIAL-002 — 탭이 없는 줄은 기존 동작을 유지한다

**Where** 줄에 탭이 없을 때, **the system shall** 기존의 공백 기준 분리로
폴백한다. adb 버전·플랫폼에 따라 구분자가 다를 가능성을 관측으로 배제하지
않았으므로, 탭 전용으로 좁히지 않는다.
...
| AC | 기준 | 판정 |
|---|---|---|
| **AC-SERIAL-001** | 공백 포함 serial 픽스처에서 `serial`이 온전히 보존되고 `state === "device"`다 | **U** 실기기 `od -c` 관측 그대로의 픽스처 사용 |
| **AC-SERIAL-002** | 공백 없는 기존 픽스처 6종이 전부 그대로 통과한다 (회귀 없음) | **U** 기존 테스트 무수정 통과 |
| **AC-SERIAL-003** | 탭이 없고 공백만 있는 줄에서도 기존과 동일하게 파싱된다 | **U** 폴백 경로 픽스처 |
| **AC-SERIAL-004** | `-l` 롱포맷의 `model:` 추출이 공백 포함 serial에서도 동작한다 | **U** `model:` 필드는 state 뒤에 온다 |
| **AC-SERIAL-005** | 빈 문자열·비문자열·형식 이탈 입력에서 던지지 않는다 | **U** 기존 테스트 유지 |
| **AC-SERIAL-006** | **실기기에서 mDNS 이름 충돌 상태의 기기가 `devices`에 `device`로 나오고 명령이 실제로 동작한다** | **D** 필수. **U 불가** — adb의 실제 출력 형태가 판정 대상이다 |
| **AC-SERIAL-007** | `pnpm test` / `pnpm typecheck` / `pnpm build` 전부 통과 | **G/U** exit 0 |

**AC-SERIAL-006 재현 조건의 위험**: mDNS 이름 충돌을 의도적으로 만드는 절차가
확립돼 있지 않다. 유발하지 못하면 **AC-VISION-004와 같은 방식으로 「명시적
미검증」으로 기록하고 닫는다** — U 통과를 D 판정으로 올리지 않는다.
